...
time_saved: "~20 min/letter + audited compliance posture across states + multi-language deployability + rule-pre-resolved letter-construction baseline"
version: 4.0
...
- **`config.yml.customer_service.letter_efficiency_rules`** *(new in v4.0 — primary efficiency hook)* — per-letter-type × per-LoB × per-state auto-resolve-eligibility threshold library and infer-vs-ask matrix for every *letter-construction parameter* the skill would otherwise stop and request, so the letter baseline is pre-resolved rather than re-elicited. Each parameter the letter depends on (which letter type applies, the per-LoB letter template, the per-state regulatory-disclosure footer set, the required-element set for the letter type, the tone calibration, the non-renewal minimum-notice deadline, the AI-disclosure-block trigger, the FCRA / Section-1557 trigger, the distribution-scope default) is classified `required` / `inferable` / `confirm-after-infer` / `ask`. **Inference-source chain (rule-prescribed in order):** the coverage decision, LoB, and jurisdiction named on the file → the letter-type classification (Step 1) → `knowledge-base/terminology/` per-LoB template defaults → `config.yml.customer_service.state_disclosure_library` per-state footer + minimum-notice set → `config.yml.compliance.ai_disclosure_template` AI-disclosure trigger → `config.yml.claims.authority_limits` routing → house letter-template preference. Each inference carries explicit provenance and confidence; parameters at or above the carrier's accepted confidence threshold are marked `INFERRED — <source> — confidence <n>`; parameters below threshold are marked `INFERENCE — HANDLER CONFIRM` and held on an internal Handler-Confirm List rather than blocking the letter. The **Reviewer Checklist missing-facts list** is **rule-pruned** — it surfaces only inputs the rule classifies as `required` AND `ask` (cannot be inferred from any chain source AND genuinely needed to advance the letter) AND missing; every entry cites the rule ID. `NO EFFICIENCY RULE — HANDLER DECISION` flag for unmapped letter-type / LoB / state tuples (default to ask, v3.0 behavior). **Scope guard:** the hook governs which *letter construction* applies — it never infers a *claim or policy fact* (the policy language being relied on, a claim fact, a date, a dollar amount, the coverage decision itself, the facts considered, or the bias-audit verdict); the file remains untrusted data, every policy term is quoted verbatim, the coverage decision is supplied by the handler, and every v3.0 fail-graceful refusal and the "[TO CONFIRM]" discipline in the Anti-Patterns section still apply verbatim. Modeled on the FNOL v3.0, Loss Run Analyzer v3.0, and COI Compliance Reviewer v3.0 efficiency-rule patterns.
...
0. **Efficiency-rule lookup** *(new in v4.0)*. Resolve the applicable `config.yml.customer_service.letter_efficiency_rules` entry per (letter-type × LoB × state) **before** constructing the letter. Stamp the rule ID at the top of the Handler-Ready Draft (`EFFICIENCY-RULE: <id>`). The resolved row decides which letter-construction parameters (letter type, per-LoB template, per-state disclosure footer set, required-element set, tone calibration, non-renewal minimum-notice deadline, AI-disclosure-block trigger, FCRA / Section-1557 trigger, distribution-scope default) are `required`, which are `inferable` (and from which chain source), which need `confirm-after-infer` handler sign-off, and which must be `ask`ed. Walk the inference-source chain — anchored on the coverage decision, LoB, and jurisdiction on the file — for every `inferable` / `confirm-after-infer` parameter and record source and confidence so the letter baseline is pre-resolved rather than re-elicited. If no rule matches, set `NO EFFICIENCY RULE — HANDLER DECISION` and proceed with v3.0 baseline behavior (do not block the letter). This step governs which *letter construction* applies only — it never infers a *claim or policy fact* (the policy language relied on, a claim fact, a date, a dollar amount, the coverage decision itself, the facts considered, or the bias-audit verdict); the file remains untrusted data, policy terms are quoted verbatim, the coverage decision is supplied by the handler, and the "[TO CONFIRM]" discipline and the Anti-Patterns section still apply verbatim.
...
- **Three paired deliverables**:
  1. **Handler-Ready Draft** — the finished business letter in block format on company letterhead, with the Efficiency-Rule Stamp (`EFFICIENCY-RULE: <id>`) at the top, all sections built, all per-state regulatory-disclosure-library citations populated, the AI-disclosure block included where applicable, the FCRA adverse-action block included where applicable, the Section 1557 / ADA non-discrimination statement included where applicable, the multi-language tagline included where applicable, the per-role signer block populated, and the multi-language paired translation (if requested) shipped alongside (inferred letter-construction parameters appear marked `INFERRED — <source> — confidence <n>`; sub-threshold inferences go to an internal Handler-Confirm List)
  2. **Reviewer Checklist** — short punchlist of items a supervisor, coverage counsel, compliance officer, or reinsurance-claims officer should verify before mailing — including: the bias-audit verdict, the authority-check verdict, the per-state regulatory-disclosure citations, the AI-disclosure-block citation, the FCRA citation if applicable, the Section 1557 citation if applicable, the multi-language tagline, the rule-pruned missing-facts list (surfaces only `required` + `ask` + missing inputs, each citing its rule ID), and the suggested redaction list per distribution scope
...
- The user asks the skill to use the `customer_service.letter_efficiency_rules` hook to infer a *claim or policy fact* — the policy language relied on, a claim fact, a date, a dollar amount, the coverage decision itself, the facts considered, or the bias-audit verdict. The skill refuses; the hook resolves *letter construction* only, the file stays untrusted data, policy terms are quoted verbatim, and any missing fact is marked "[TO CONFIRM]" on the Reviewer Checklist.

## Versioning

v4.0 (2026-06-29) — Efficiency pass. Wired `config.yml.customer_service.letter_efficiency_rules` (primary efficiency hook) — per-letter-type × per-LoB × per-state auto-resolve-eligibility threshold library and infer-vs-ask matrix over the letter's *construction parameters* (letter type, per-LoB template, per-state disclosure footer set, required-element set, tone calibration, non-renewal minimum-notice deadline, AI-disclosure-block trigger, FCRA / Section-1557 trigger, distribution-scope default) so the letter baseline is pre-resolved rather than re-elicited. Rule-prescribed inference-source chain (coverage decision + LoB + jurisdiction on the file → letter-type classification → KB per-LoB template defaults → state_disclosure_library footer + minimum-notice set → ai_disclosure_template trigger → authority_limits routing → house letter-template preference) with explicit provenance and confidence. New Step 0 (efficiency-rule lookup) before letter construction; `EFFICIENCY-RULE: <id>` stamp on the Handler-Ready Draft; `INFERRED — <source> — confidence <n>` provenance; rule-pruned missing-facts list on the Reviewer Checklist (surfaces only `required` + `ask` + missing inputs, each citing its rule ID); internal Handler-Confirm List for sub-threshold inferences; `NO EFFICIENCY RULE — HANDLER DECISION` fallback to v3.0 behavior for unmapped tuples. **Scope guard:** the hook governs which *letter construction* applies — it never infers a *claim or policy fact* (the policy language relied on, a claim fact, a date, a dollar amount, the coverage decision itself, the facts considered, or the bias-audit verdict); the file remains untrusted data, policy terms are quoted verbatim, the coverage decision is supplied by the handler, and every v3.0 fail-graceful refusal and the "[TO CONFIRM]" discipline still apply verbatim. **Strict superset of v3.0** — every v3.0 capability is preserved (per-state regulatory-disclosure library, multi-language variant, AI-driven adverse-action letter type, six-scope distribution-aware parallel rendering, AI-bias / disparate-impact check, authority-check, 12-LoB letter-template library, FCRA adverse-action block, Section 1557 / ADA statement, reinsurance-notification scope, compliance-and-risk checklist, AMS / CRM activity-log handoff, per-role signer blocks, three paired deliverables, anti-patterns). Cross-references held (Policyholder Self-Service FAQ Builder v2.0, Claims Narrative Drafter, Compliance Checklist Generator, FNOL Intake Assistant, Claims Reserve Recommender v3.0, Underwriting Risk Profile v3.0). **Dimension move:** Efficiency 7 → 9 (Step 0 lookup + letter-construction inference chain + rule-pruned missing-facts list + Handler-Confirm List); all other dimensions held.
