...
- **`config.yml.underwriting.loss_run_efficiency_rules`** *(new in v3.0 — primary efficiency hook)* — per-LoB × per-state × per-class auto-resolve-eligibility threshold library and infer-vs-ask matrix for every analysis input the skill would otherwise stop and request. Each input (development pattern, carrier reporting convention, NAICS/class bridge, prior-period loss-run, valuation date, ULAE convention, ex-mod baseline, benchmark loss ratio, large-loss threshold) is classified `required` / `inferable` / `confirm-after-infer` / `ask`. **Inference-source chain (rule-prescribed in order):** carrier's own development pattern (when shipped on the loss-run) → prior-period loss-run for the same account → `knowledge-base/terminology/development-patterns/` LoB-default triangle → AMS/aggregator export field map → ISO / NCCI experience-mod feed → carrier reporting-convention preset (`loss_run_format_presets`) → industry benchmark feed (ISO / NCCI / IBC). Each inference carries explicit provenance and confidence; inputs at or above the carrier's accepted confidence threshold are marked `INFERRED — <source> — confidence <n>`; inputs below threshold are marked `INFERENCE — ANALYST CONFIRM` and held for the analyst rather than blocking the analysis. The **Open Questions** list is **rule-pruned** — it asks only for inputs the rule classifies as `required` AND `ask` (cannot be inferred from any chain source AND genuinely needed to advance the analysis) AND missing; every entry cites the rule ID. `NO EFFICIENCY RULE — ANALYST DECISION` flag for unmapped LoB / state / class tuples (default to ask). This never overrides the existing fail-graceful behavior: a fabricated incurred amount, claim count, or undocumented development tail is still refused — inference is permitted only where the rule classifies the input as `inferable` / `confirm-after-infer` AND a chain source delivers a confidence above the carrier's threshold AND the input is a methodology parameter, not a source data value. Inspired by the FNOL v3.0 and Submission Intake Summarizer v4.0 efficiency-rule patterns.
...
0. **Efficiency-rule lookup** *(new in v3.0)*. Resolve the applicable `config.yml.underwriting.loss_run_efficiency_rules` entry per (LoB × state × class) **before** parsing. Stamp the rule ID at the top of the Master Analysis (`EFFICIENCY-RULE: <id>`). The resolved row decides which methodology parameters (development pattern, valuation date, ULAE convention, ex-mod baseline, benchmark loss ratio, large-loss threshold) are `required`, which are `inferable` (and from which chain source), which need `confirm-after-infer` analyst sign-off, and which must be `ask`ed. Walk the inference-source chain for every `inferable` / `confirm-after-infer` parameter and record source and confidence. If no rule matches, set `NO EFFICIENCY RULE — ANALYST DECISION` and proceed with v2.0 baseline behavior (do not block the analysis). This step governs methodology parameters only — source data values (incurred amounts, claim counts, development patterns the loss-run does not support) are never inferred and the fail-graceful flags in step 1 and the Anti-Patterns section still apply verbatim.
...
- Three paired deliverables:
  1. **Master Analysis** — markdown report with sections in this order: Efficiency-Rule Stamp (`EFFICIENCY-RULE: <id>`) · Executive Summary · Key Metrics · Per-LoB Metric Library · Trend Analysis · Large Losses · IBNR Projection · Reserve Adequacy · AI-Bias and Authority Audit · Insights and Recommendations · Producer Talk Track · Open Questions (rule-pruned per Step 0 — asks only for `required` + `ask` + missing methodology inputs, each citing its rule ID; inferred parameters appear in the methodology footnote marked `INFERRED — <source> — confidence <n>`, and sub-threshold inferences go to an internal **Analyst-Confirm List** rather than the Open Questions) · Sources cited inline
...
v3.0 (2026-06-08) — Efficiency pass. Wired `config.yml.underwriting.loss_run_efficiency_rules` (primary efficiency hook) — per-LoB × per-state × per-class auto-resolve-eligibility threshold library and infer-vs-ask matrix over the analysis's *methodology parameters* (development pattern, valuation date, ULAE convention, ex-mod baseline, benchmark loss ratio, large-loss threshold). Rule-prescribed inference-source chain (carrier's own development pattern → prior-period loss-run → KB LoB-default triangle → AMS/aggregator export map → ISO/NCCI experience-mod feed → reporting-convention preset → industry benchmark feed) with explicit provenance and confidence. New Step 0 (efficiency-rule lookup) before parsing; `EFFICIENCY-RULE: <id>` stamp on the Master Analysis; `INFERRED — <source> — confidence <n>` provenance in the methodology footnote; rule-pruned Open Questions list (asks only for `required` + `ask` + missing methodology inputs, each citing its rule ID); internal Analyst-Confirm List for sub-threshold inferences; `NO EFFICIENCY RULE — ANALYST DECISION` fallback to v2.0 behavior for unmapped tuples. **Scope guard:** the hook governs methodology parameters only — source data values (incurred amounts, claim counts, unsupported development tails) are never inferred; every v2.0 fail-graceful flag and every Anti-Pattern refusal still applies verbatim. **Strict superset of v2.0** — every v2.0 capability is preserved (12-carrier + 4-AMS format presets, per-LoB metric library, parameterized IBNR projection, reserve-adequacy traffic light, AI-bias / disparate-impact check, authority-check, producer talk-track, distribution-scoped parallel rendering, AMS activity-log handoff, per-role signer blocks, anti-patterns). Cross-references updated to Underwriting Risk Profile v3.0, Renewal Review Brief v4.0, Submission Intake Summarizer v4.0. **Dimension move:** Efficiency 7 → 9 (Step 0 lookup + methodology-parameter inference chain + rule-pruned Open Questions + Analyst-Confirm List); all other dimensions held.
