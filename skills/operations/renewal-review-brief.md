...
- Load `config.yml` from the repo root for:
  ...
  - **`agency.renewal_efficiency_rules`** *(new in v4.0 — primary efficiency hook)* — per-LoB × per-state × per-class × per-retention-tier auto-populate-eligibility threshold library and infer-vs-ask matrix for every input the brief would otherwise stop and request at T-90. Each input (expiring-program detail, loss-history snapshot, market-cycle posture, carrier-appetite read, rate-trend band, account notes, audit posture) is classified `required` / `inferable` / `confirm-after-infer` / `ask`. **Inference-source chain (rule-prescribed in order):** expiring-policy / declarations record → upstream Loss Run Analyzer v3.0 verdict → upstream Underwriting Risk Profile v3.0 score → AMS/BMS account record → `carrier_appetite_map` + `contingency_posture` config → market-rate-trend feed (class/line) → prior renewal brief for the same account. Each inference carries explicit provenance and confidence; inputs at or above the carrier's accepted confidence threshold are marked `INFERRED — <source> — confidence <n>`; inputs below threshold are marked `INFERENCE — PRODUCER CONFIRM` and held for the producer rather than fired off as a T-90 account-data-refresh request. The **T-90 account-data-refresh request list** (Section 5) is **rule-pruned** — it asks the insured / broker only for inputs the rule classifies as `required` AND `ask` (cannot be inferred from any chain source AND genuinely needed to set the verdict) AND missing; every request cites the rule ID. `NO EFFICIENCY RULE — PRODUCER DECISION` flag for unmapped tuples (default to ask). The intent: where the Loss Run Analyzer and Underwriting Risk Profile have already run on the account, the brief self-populates Sections 2 and 3 from their outputs and the expiring-program record, so the T-90 refresh request shrinks to the genuinely-missing, genuinely-insured-attested residue. Inspired by the FNOL v3.0 and Submission Intake Summarizer v4.0 efficiency-rule patterns.
...
Resolve the applicable `agency.renewal_efficiency_rules` entry per (LoB × state × class × retention-tier) **before** building the snapshot. Stamp the rule ID in the brief header (`EFFICIENCY-RULE: <id>`). The resolved row decides which inputs are `required`, which are `inferable` (and from which chain source), which need `confirm-after-infer` producer sign-off, and which must be `ask`ed at T-90. Before sending any account-data-refresh request, consume the upstream **Loss Run Analyzer v3.0** verdict and **Underwriting Risk Profile v3.0** score if they have already run on this account, and walk the inference-source chain for every `inferable` / `confirm-after-infer` input — record source and confidence. If no rule matches, set `NO EFFICIENCY RULE — PRODUCER DECISION` and proceed with v3.0 baseline behavior (do not block the brief).
...
|---|---|---|---|
| **T-90** | Brief delivered to producer; account-data refresh requests sent (**rule-pruned per Step 0** — only `required` + `ask` + missing inputs, each citing its rule ID; inferred inputs already self-populated and marked `INFERRED — <source> — confidence <n>`; sub-threshold inferences on an internal Producer-Confirm List, not sent to the insured) | Producer + CSR | Always |
...
- v4.0 (this version) — Efficiency pass. Wired `config.yml.agency.renewal_efficiency_rules` (primary efficiency hook) — per-LoB × per-state × per-class × per-retention-tier auto-populate-eligibility threshold library and infer-vs-ask matrix over every T-90 input (expiring-program detail, loss-history snapshot, market-cycle posture, carrier-appetite read, rate-trend band, account notes, audit posture). Rule-prescribed inference-source chain (expiring-policy record → Loss Run Analyzer v3.0 verdict → Underwriting Risk Profile v3.0 score → AMS/BMS record → carrier_appetite_map + contingency_posture → market-rate-trend feed → prior renewal brief) with explicit provenance and confidence. New Step 0 (efficiency-rule lookup) before the snapshot; `EFFICIENCY-RULE: <id>` stamp in the brief header; `INFERRED — <source> — confidence <n>` provenance on self-populated inputs; rule-pruned T-90 account-data-refresh request (asks the insured / broker only for `required` + `ask` + missing inputs, each citing its rule ID); internal Producer-Confirm List for sub-threshold inferences; `NO EFFICIENCY RULE — PRODUCER DECISION` fallback to v3.0 behavior for unmapped tuples. **Strict superset of v3.0** — every v3.0 capability is preserved (retention-tier rubric, carrier-appetite-map + contingency-posture reads, five-verdict strategy decision matrix, quantified premium/coverage/agency-revenue/risk-of-loss outcomes, T-minus action calendar with state non-renewal-notice clock, five-track producer talk-track, three paired deliverables, risk & compliance flags, distribution-scope label, all cross-references). Cross-references updated to consume Loss Run Analyzer v3.0 and Underwriting Risk Profile v3.0. **Dimension move:** Efficiency 7 → 9 (Step 0 lookup + inference-source chain + rule-pruned T-90 refresh request + Producer-Confirm List); all other dimensions held.
- v3.0 — Retention-tier rubric (Platinum / Gold / Standard / At-Risk / Decline-to-Renew) overrideable from config; carrier-appetite-map and contingency-posture pulled from `config.yml`; five-verdict strategy decision matrix (Renew As-Is / Re-Negotiate / Restructure / Remarket / Decline-to-Renew); quantified premium / coverage / agency-revenue / risk-of-loss-of-account outcome per verdict; T-minus action calendar driven by renewal effective date and state non-renewal-notice timeline; five-track producer talk-track tuned to verdict; three paired deliverables (Brief / One-Pager / AMS handoff); cross-references to Loss Run Analyzer, UW Risk Profile, Submission Intake Summarizer v3.0, Producer Live-Call Copilot, COI Compliance Reviewer, Cross-Sell Opportunity Analyzer, Compliance Checklist Generator. Distribution-scope label.
