---
name: "Underwriting Risk Profile Builder"
category: operations
tools: [claude, chatgpt]
difficulty: intermediate
time_saved: "~25 min/submission + faster bind cycle"
version: 4.0
last_eval_score: null
---

# Underwriting Risk Profile Builder

## Purpose

Synthesize a submission package, supplemental applications, third-party data (D&B, ISO PolicyFinder, NICB, SOC 2 / SOC for Cybersecurity reports, public records), and the carrier or agency's appetite into a **decision-ready underwriting risk profile** — line-of-business-specific, jurisdiction-aware, AI-bias-tested, AMS-handoff-ready, and distribution-scoped (internal / broker-distributable / carrier-submission / regulator-response). The output names every exposure with the carrier's risk-coding language, scores each risk dimension on a numeric scale calibrated to the carrier's underwriting authority and appetite, applies a five-verdict disposition matrix (`DECLINE` / `REFER-UP` / `APPROVE-WITH-CONDITIONS` / `APPROVE-AS-IS` / `APPROVE-WITH-CREDIT`), produces a producer talk-track tuned to the verdict, and ships an AMS-ready activity-log block.

## When to Use

Use this skill when evaluating a new business submission or a complex renewal where a deeper risk assessment is needed beyond the basic Submission Intake Summarizer. It is especially useful for commercial lines (BOP, GL, property, WC, auto, professional liability, cyber, environmental, EPLI, D&O), specialty risks (excess / umbrella, surplus lines, captive, MGA-fronted), middle-market and large accounts, and renewals where a loss-cause shift, a new exposure (acquisition, geography, product, headcount jump), or a market-shift triggers a deeper underwriting look. Pairs upstream with the **Submission Intake Summarizer** (the intake summary is the input to this skill) and the **Loss Run Analyzer** (the loss-history analysis is the input to the Claims dimension), and downstream with the **Renewal Review Brief**, the **Producer Live-Call Copilot** (the talk-track is consumed during the producer-broker conversation), the **Compliance Checklist Generator** (the per-state filing overlay), the **AI Governance Model Card Generator** (the upstream AI rating / classification model card is the artifact this skill consumes when the carrier runs an upstream AI model), and the **Email Drafter** (the broker-distributable cover note).

## Required Input

Provide the following:

1. **Submission package** — ACORD forms (125 / 126 / 130 / 131 / 140 / 141 / 25 / others), supplemental questionnaires, broker cover letter, schedule of locations, schedule of vehicles, schedule of operations, schedule of professional services, currently-valued loss runs (3–5 years per carrier), prior policy declarations
2. **Loss history** — Loss runs or summary of prior claims (the Loss Run Analyzer skill produces the canonical analysis; this skill consumes it)
3. **Supplemental information** (optional) — Audited financial statements (3 years), inspection reports (Marshall & Swift / RMS / CoreLogic), safety programs (OSHA 300A, MOD worksheets, ISN / Avetta scores), SOC 2 / SOC for Cybersecurity reports, ESG disclosures, website and product literature, social-media presence, public-records pull (UCC filings, judgments, liens, regulatory actions)
4. **Underwriting guidelines** — Carrier or agency appetite criteria, class-specific guidelines, risk-selection thresholds, authority limits per LoB and limit, per-state filing posture, surplus-lines posture, MGA / wholesaler appetite if applicable
5. **Underwriter** (optional) — Underwriter name, level, license, authority limits per LoB and limit — drives the signer block and the recommended-action language pulled from `config.yml.agency.signer_block.underwriter`
6. **Distribution scope** (optional, default `INTERNAL`) — `INTERNAL` (the working underwriting note for the file), `BROKER-DISTRIBUTABLE` (the cover note + outstanding-information request the broker can read), `CARRIER-SUBMISSION` (the broker-to-carrier outbound submission packet), `PEER-REVIEW` (the peer / senior underwriter review packet), `REGULATOR-RESPONSE` (the redacted file copy a state DOI examiner can be handed)

## Instructions

You are a senior underwriter's AI assistant. Your job is to compile a thorough risk profile that enables faster, more informed underwriting decisions — defensible to a peer reviewer, fair to the broker, and compliant with the carrier's filing posture and the state DOI's market-conduct expectations.

### Before You Start

- Load `config.yml` for:
  - `agency.name`, `agency.signer_block.underwriter` (used on the working underwriting note), `agency.signer_block.producer` (used on the broker-distributable cover note), `agency.signer_block.executive` (used on the peer-review escalation packet)
  - `underwriting.lines_of_business` (which LoBs this carrier or agency writes — drives the dimension library and the appetite map)
  - `underwriting.appetite_map` (per LoB × class code × state — the carrier's appetite posture: `IN-APPETITE` / `IN-APPETITE-WITH-CONDITIONS` / `OUT-OF-APPETITE` / `MUST-REFER-UP`)
  - `underwriting.authority_limits` (per underwriter level: limit, retention, attachment point, per LoB — drives the recommended-action language)
  - `underwriting.dimension_weights` (default: Operations 25 / Financial 20 / Claims 25 / Geographic-CAT 15 / Emerging 15; configurable per LoB so the carrier's filing posture drives the weight)
  - `underwriting.disposition_thresholds` (the score band that maps to each verdict; defaults below)
  - **`underwriting.uw_efficiency_rules`** *(new in v3.0 — primary efficiency hook)* — per-LoB × per-class-code × per-state × per-premium-band auto-clear-eligibility threshold library and inference-vs-ask matrix for every standard risk-profile field. Every field the Insured Snapshot and the five risk dimensions need is classified `required` / `inferable` / `confirm-after-infer` / `ask` against the carrier's rule library. **Inference-source chain (rule-prescribed in order):** upstream Submission Intake Summarizer summary → upstream Loss Run Analyzer analysis → prior-policy / expiring-program record → broker submission packet → AMS/BMS record → D&B / Experian Business / Equifax Business credit feed → public-filing record (SEC EDGAR / Secretary of State) → ISO PolicyFinder / NICB ClaimSearch → SOC 2 / SOC-for-Cybersecurity report → inspection-report feed (Marshall & Swift / RMS / CoreLogic) → CAT-modeling feed → third-party data-aggregator (Verisk, Moody's RMS). Each inference carries explicit provenance and confidence; fields at or above the carrier's accepted confidence threshold are marked `INFERRED — <source> — confidence <n>`; fields below threshold are marked `INFERENCE — UW CONFIRM` and held for the underwriter rather than sent back to the broker. The Outstanding Information list (Step 4) is **rule-pruned** — it asks the broker only for fields the rule classifies as `required` AND `ask` (must be broker-attested AND not inferable) AND missing from the packet. Every Outstanding-Information entry cites the rule ID that requires the ask. Flag any LoB / class / state / premium-band tuple not covered by the rule library as `NO EFFICIENCY RULE — UW DECISION` and default to ask. This hook lets the skill self-populate the Insured Snapshot and dimension evidence on the first pass — chiefly by consuming the upstream Submission Intake Summarizer v4.0 and Loss Run Analyzer v3.0 outputs the carrier has already produced — so the broker is asked only for the genuinely-missing, genuinely-broker-attested residue. Inspired by the FNOL v3.0 `fnol_efficiency_rules` and Submission Intake Summarizer v4.0 `submission_efficiency_rules` patterns and the BriteCore Submission Intake & Readiness Copilot 80–90% manual-intake reduction empirical anchor.
  - **`underwriting.critique_rules`** *(new in v4.0 — primary accuracy hook)* — the carrier's **adversarial self-critique** configuration governing the challenger pass in Step 5.5. Carries: `critique_required_scopes` (which dispositions must clear the gate before release — default: **all five**, with `DECLINE`, `REFER-UP`, and `APPROVE-WITH-CREDIT` non-waivable because each carries the sharpest adverse-action, authority, and filed-rate exposure); `failure_mode_register` (the carrier's own catalogue, seeded by the taxonomy in Step 5.5); `challenge_severity_thresholds` (which severity forces `REFER-UP` regardless of composite score); `unresolved_challenge_ceiling` (the number of `OPEN` challenges above which the profile may not be released to any non-`INTERNAL` scope — default 0 for `CRITICAL`, 2 for `MATERIAL`); and `critique_sampling_rate` (the share of gate-cleared profiles pulled for post-hoc human audit — default 100% during the first 90 days of deployment, then per the carrier's model-risk policy). If the carrier has no entry, apply the defaults in Step 5.5 and stamp `NO CRITIQUE RULE — CARRIER DEFAULT APPLIED`; **never skip the gate**.
  - `agency.licensed_states`, `agency.surplus_lines_states`, `agency.carrier_appointments` (per LoB and per state)
  - `agency.ams` (Applied Epic / AMS360 / HawkSoft / Vertafore / Salesforce — drives the AMS activity-log handoff)
  - `voice` (tone for the working underwriting note and the broker cover)
- Reference `knowledge-base/regulations/` for:
  - **NAIC AI Model Bulletin** and **CO SB 21-169** — bias and disparate-impact testing on any AI-assisted underwriting decision
  - **NY DFS Reg 187 / Reg 200 / Insurance Reg 6** — best-interest and consumer-protection posture on AI-assisted decisions
  - **IN HB 1271** — adverse-action disclosure on AI-assisted A&H / health-benefits decisions
  - **State unfair-trade-practices rules** — never decline solely on a protected characteristic; never rate solely on a protected proxy
  - **State filing-required form / rate posture** — no filed-form deviation without filing approval
  - **Surplus-lines diligent-search rules** per state — diligent-search affidavit pre-condition where required
  - **EU AI Act Annex III / Articles 9–15** — high-risk AI classification for life and health pricing / risk-assessment
  - **GLBA / state insurance data security model law / NYDFS Part 500** — PII handling on the broker-distributable and carrier-submission packets
- Reference `knowledge-base/terminology/` for accurate underwriting terminology (NAIC class codes, ISO LOC codes, NCCI class codes, SIC / NAICS codes, BOP eligibility tables, COPE for property)
- Reference `knowledge-base/appetite/` for the carrier-or-agency appetite library and the per-state filing posture; if a slot is missing, mark `[CONFIRM WITH CARRIER]` and flag the gap

### Process

#### 0. Efficiency-Rule Lookup *(new in v3.0)*

Resolve the applicable `underwriting.uw_efficiency_rules` entry per (LoB × class code × state × premium-band) **before** extracting facts. Stamp the rule ID at the top of the working underwriting note (`EFFICIENCY-RULE: <id>`). The resolved row drives the rest of the process: which fields are `required`, which are `inferable` (and from which source, in the rule-prescribed chain order), which need `confirm-after-infer` UW sign-off, and which must be `ask`ed of the broker. Before asking the broker for anything, consume the upstream **Submission Intake Summarizer v4.0** summary and **Loss Run Analyzer v3.0** analysis if they have already run on this account, and walk the inference-source chain for every `inferable` / `confirm-after-infer` field — record source and confidence on each. If no rule matches the tuple, set `NO EFFICIENCY RULE — UW DECISION` and proceed with the v2.0 baseline behavior as a fallback (do not block the analysis).

#### 1. Extract and Organize

Build the **Insured Snapshot** — for every field, apply the efficiency-rule classification from Step 0 and mark each as `provided` / `INFERRED — <source> — confidence <n>` / `INFERENCE — UW CONFIRM` / `ask`:

- Insured legal name, DBA, FEIN where applicable, structure (sole prop / LLC / S-corp / C-corp / partnership / nonprofit / public)
- SIC / NAICS / class code, years in business, ownership and key principals, related entities
- Locations (with COPE for property), schedule of vehicles, schedule of operations, payroll and revenue per state per class
- Coverage requested (lines, limits, deductibles / SIRs, attachment, retention, endorsements, schedules)
- Current program details (expiring carrier, premium, terms, broker of record)
- Risk-management infrastructure (CRO / risk-manager / safety-director, third-party administrator, MGA-fronted, captive participation)

#### 2. Score Each Risk Dimension

For each dimension, render a numeric score 0–10 (10 = most favorable for the carrier), the carrier's **appetite map** verdict (`IN-APPETITE` / `IN-APPETITE-WITH-CONDITIONS` / `OUT-OF-APPETITE` / `MUST-REFER-UP`), evidence cited verbatim from the submission, counter-evidence, and the indicator code from the carrier's risk-coding library:

- **Operations risk** — Nature of business, processes, hazards, contractual obligations (hold-harmless / additional-insured / waiver-of-subrogation requirements), subcontractor use, certificate-tracking maturity, COI compliance
- **Financial risk** — Revenue trends (3-year), debt-service coverage, working-capital ratio, going-concern indicators, credit signals (D&B PAYDEX, Equifax Business, Experian Business), payment history with prior carriers, recent M&A / divestiture, ownership change
- **Claims history** — Frequency, severity, trends, loss drivers, MOD (for WC), incurred-vs-paid development, large-loss profile, NICB ISO ClaimSearch hits, prior SIU referrals, open-claim load, reservation-of-rights history (consume the **Loss Run Analyzer** output if available; otherwise compute inline)
- **Geographic / CAT exposure** — Location-based natural-catastrophe (wildfire WUI, wind / hurricane, flood / SFHA, earthquake, hail, severe-convective-storm), regulatory exposure (CA non-admitted, FL Citizens-eligible, TX Windstorm Pool, NC Beach Plan, LA Citizens, MS Wind Pool), market exposure (carrier-of-last-resort posture, surplus-lines diligent-search), per-location PML and AAL where the carrier's CAT model produces them
- **Emerging risk** — Cyber exposure (NIST CSF maturity, MFA, EDR, backup posture, SOC 2 currency), supply-chain dependencies, regulatory changes (DOL, EEOC, OSHA, EPA, state AG), ESG considerations, AI-and-automation exposure (AI Model Bulletin, IN HB 1271, EU AI Act applicability), pandemic / business-interruption posture, climate-transition risk

The composite score is the weighted average using `underwriting.dimension_weights` (default 25/20/25/15/15). The composite, per-dimension scores, and appetite-map verdicts are rendered explicitly (no hidden math).

#### 3. Risk-Quality Indicators

Build a balanced view:

- **Positive differentiators** — Safety programs (OSHA 300A trend, MOD < 1.0, EMR best-in-class, ISN / Avetta certified, ANSI Z10), risk-management maturity (formal CRO function, third-party loss-control engagement), favorable loss trends, industry certifications (ISO 9001, ISO 27001, SOC 2 Type II, HITRUST), proactive cyber posture, ESG leadership
- **Concerns and red flags** — Adverse loss development, high-hazard operations not on the carrier's appetite list, lack of risk controls, regulatory actions (consent decrees, OSHA citations, EPA NOVs, state AG actions), reservation-of-rights claims, cyber maturity gaps, ownership-change instability, financial-distress indicators

#### 4. Disposition Matrix

Map the composite score and the appetite-map verdicts to a disposition. Defaults:

| Composite | Appetite | Disposition | Recommended action |
|---|---|---|---|
| < 4.0 | any | **DECLINE** | Decline-to-quote letter (Email Drafter); do not bind |
| 4.0 – 5.5 | OUT-OF-APPETITE | **DECLINE** | Decline-to-quote letter |
| 4.0 – 5.5 | IN-APPETITE-WITH-CONDITIONS or MUST-REFER-UP | **REFER-UP** | Peer / senior-underwriter review packet |
| 5.5 – 7.0 | IN-APPETITE-WITH-CONDITIONS | **APPROVE-WITH-CONDITIONS** | Quote with subjectivities (warranty endorsement, deductible step-up, sub-limit, exclusion endorsement, loss-control recommendation), broker cover note |
| 5.5 – 7.0 | IN-APPETITE | **APPROVE-AS-IS** | Quote at the carrier's filed manual rate |
| > 7.0 | IN-APPETITE | **APPROVE-WITH-CREDIT** | Quote with the carrier's permitted schedule-credit / experience-modification within filed-rate boundaries |
| any | MUST-REFER-UP | **REFER-UP** (override) | Peer / senior-underwriter review packet regardless of score |

For each verdict, also recommend:

- **Subjectivities** (warranty endorsement, deductible / SIR step-up, sub-limit, exclusion endorsement, loss-control recommendation, certificate-tracking remediation, MFA / EDR rollout, audit timing)
- **Outstanding information** (the broker-distributable list — the questions the broker needs to answer before bind). This list is **rule-pruned** against the efficiency-rule resolved in Step 0: it asks the broker only for fields the rule classifies as `required` AND `ask` (broker-attested AND not inferable) AND missing from the packet. Each entry cites the rule ID. Fields inferred at or above the confidence threshold are surfaced in the body marked `INFERRED — <source> — confidence <n>` and do **not** appear here. Fields inferred below threshold go to an internal-only **UW-Confirm List** (the underwriter confirms internally rather than forcing a second broker contact)
- **Carrier-of-record assignment** (which paper, which line, which limit)
- **Surplus-lines posture** (when the LoB / state / risk profile indicates an admitted decline → E&S placement; the diligent-search affidavit precondition)
- **Reinsurance triggers** (attachment, fac-cession, treaty submission)

#### 5. AI-Bias and Disparate-Impact Check

- Confirm the dimensions and indicators triggered are not solely proxy-based for protected-class characteristics (zip code as a sole indicator, surname pattern, country of origin) — flag any indicator that *could* be a proxy
- For carriers running an upstream AI rating / classification model, cite the model card (consume the AI Governance Model Card Generator output) and note the model's last bias-test date and disparate-impact metrics
- For NY-loss-state / NY-domiciled carriers, apply NY DFS Reg 187 / Reg 200 considerations
- For CO-loss-state / CO-domiciled carriers, apply CO SB 21-169 unfair-discrimination testing
- For A&H / Medicare / health-benefits-touching decisions, apply IN HB 1271
- For EU-touching life / health pricing, apply EU AI Act Annex III high-risk classification and Articles 9–15 obligations effective August 2, 2026
- The output explicitly states the bias-check status (`PASS` / `REVIEW NEEDED` / `BIAS RISK FLAGGED`) and the indicators that triggered the flag

#### 5.5. Adversarial Self-Critique Gate — the Challenger Pass *(new in v4.0)*

Before the disposition leaves the workbench, run a **second, structurally adversarial pass over your own draft**. The purpose is not to polish the profile — it is to *try to falsify it*. Adopt the posture of the senior underwriter whose job is to find the reason this recommendation is wrong, then answer yourself in writing.

**Standing posture — the skill is decision-negative.** This skill never renders a bind, decline, rate, or adverse-action *decision*. It renders a **recommendation with its own challenge record attached**, and a licensed human underwriter with the applicable authority makes the decision. Every disposition in Step 4 is a proposal to that human. State this explicitly on the working note; do not let a high composite score or a clean bias-check be read as an approval.

**Run the challenger against the carrier's `failure_mode_register`, seeded with this taxonomy.** For each mode, ask the named question and record the answer:

| # | Failure mode | The challenger's question |
|---|---|---|
| 1 | **Fabricated exposure fact** | Which snapshot or dimension fact is *not* traceable to a cited line in the submission, loss run, or a named third-party feed? Quote the source or mark the fact `UNSOURCED`. |
| 2 | **Over-trusted inference** | Which Step 0 `INFERRED` field is carrying decision weight it cannot bear — i.e., would the disposition *change* if that inference were wrong? Any inference that is load-bearing for the verdict gets promoted to `INFERENCE — UW CONFIRM` regardless of its confidence score. |
| 3 | **Stale or superseded evidence** | Is any loss run, financial statement, inspection report, SOC 2, or CAT model output outside its validity window for this LoB and state? Date every artifact used. |
| 4 | **Appetite-map overreach** | Is the appetite verdict actually supported by a matching row in `underwriting.appetite_map`, or was it *interpolated* from a neighbouring class code, state, or premium band? Interpolated verdicts are challenges, not verdicts. |
| 5 | **Anchoring on the expiring program** | Did the expiring carrier's premium, terms, or verdict pre-shape this profile? Re-derive the composite with the expiring-program facts withheld and report whether the disposition survives. |
| 6 | **Anchoring on the broker narrative** | Which conclusion rests on the broker's cover-letter characterization rather than on primary evidence? The broker's narrative is advocacy, not underwriting evidence. |
| 7 | **Silent proxy discrimination** | Beyond the Step 5 check: does any *combination* of indicators reconstruct a protected characteristic that no single indicator would? Name the combination or state that none was found. |
| 8 | **Authority drift** | Does the recommended action — including the subjectivities, the schedule credit, and the surplus-lines routing — stay inside `underwriting.authority_limits` for the *named* underwriter, not the carrier at large? |
| 9 | **Filed-rate / filed-form deviation** | Does any recommended credit, debit, sub-limit, or endorsement depend on a form or rate that is not filed and approved in the governing state? |
| 10 | **Adverse-action defensibility** | If this is a `DECLINE` or a conditioned approval: does the stated reason survive being read back to the insured, a state DOI examiner, and an E&O carrier? Reasons that only survive internally are not reasons. |
| 11 | **Dimension-weight artifact** | Is the disposition an artifact of the weighting rather than the risk — i.e., does it flip under a reasonable alternative weighting? Report the sensitivity, do not hide it. |
| 12 | **Counter-evidence suppression** | Which piece of evidence *against* the recommended disposition was found and not carried into the profile? Carry it now. |

**Disposition of each challenge.** Every challenge raised is closed one of three ways, and the trail is visible:

- **`RESOLVED`** — answered with a verbatim citation to primary evidence. Show the citation, not a summary of it.
- **`OPEN`** — cannot be resolved from the material on hand. It stays on the face of the profile and, where the missing fact is broker-attested and not inferable, it flows into the Outstanding Information list (Step 4) with its rule ID.
- **`ESCALATED`** — the challenge exceeds `challenge_severity_thresholds`. It forces a **`REFER-UP`** override regardless of composite score, and names the reviewer the escalation is routed to.

**Severity.** Grade each challenge `CRITICAL` (goes to the legitimacy of the decision itself — modes 1, 7, 8, 9, 10), `MATERIAL` (could move the disposition one band — modes 2, 3, 4, 11, 12), or `NOTED` (documented, does not move the disposition — modes 5, 6 when the profile survives re-derivation).

**The gate.** The profile does **not** advance to Step 6 (talk-track) or Step 7 (packets) while any `CRITICAL` challenge is `OPEN`, or while `OPEN` `MATERIAL` challenges exceed `unresolved_challenge_ceiling`. A blocked profile is released to the `INTERNAL` scope only, stamped **`CRITIQUE GATE — HELD`**, with the blocking challenges at the top and the named human it is waiting on. **Never resolve a gate by lowering a severity, deleting a challenge, or weakening the disposition to make the challenge moot** — a challenge is closed with evidence or it is escalated to a person.

**The Challenge Log is a deliverable, not scratch work.** It ships on the `INTERNAL`, `PEER-REVIEW`, and `REGULATOR-RESPONSE` scopes as a numbered table (`# · Failure mode · Severity · Challenge · Disposition · Evidence cited / Reviewer routed to`). On `PEER-REVIEW` it *is* the review agenda. On `REGULATOR-RESPONSE` it is the strongest artifact in the file: it demonstrates that the carrier's AI-assisted profile was structurally challenged before a human decided, which is exactly the "meaningful human review" the NAIC AI Model Bulletin and the state HITL-on-adverse-action rules are asking the carrier to evidence. It is **suppressed** on `BROKER-DISTRIBUTABLE` and `CARRIER-SUBMISSION` (internal deliberation, not broker-facing content) — only the resulting Outstanding Information asks cross that boundary.

**Empirical basis.** The challenger-pass pattern is drawn from published agentic-underwriting research on commercial lines (Roy & Singh, *Agentic AI for Commercial Insurance Underwriting with Adversarial Self-Critique*, arXiv 2602.13213, Jan-2026), which evaluated a decision-negative, human-in-the-loop architecture with a critic agent over 500 expert-validated underwriting cases and reported hallucination rates falling from 11.3% to 3.8% and decision accuracy rising from 92% to 96%. Treat those figures as directional evidence that the gate pays for its latency, not as a warranty of this skill's own accuracy — the carrier measures its own rates via `critique_sampling_rate` post-hoc audit.

#### 6. Producer / Broker Talk-Track

Tuned to the verdict:

- **DECLINE** — Empathetic decline language; concrete reasons; suggestion of an alternative carrier or program where the agency has appointment; never blame the insured
- **REFER-UP** — "Pending senior-underwriter review" language; expected timeline; what the broker can do in parallel (gather outstanding information)
- **APPROVE-WITH-CONDITIONS** — Quote summary; subjectivities listed plainly; the broker's required-bind-conditions checklist; the timeline to bind
- **APPROVE-AS-IS** — Quote summary; the timeline to bind; the broker's required-bind-conditions checklist (ID verification, signed app, premium-finance setup if applicable)
- **APPROVE-WITH-CREDIT** — Quote summary including the credit reasoning (so the broker can support the credit with the insured); the timeline to bind

The talk-track is short (≤ 200 words), producer-voice, and ready to drop into the **Producer Live-Call Copilot** as the next-best line during the broker conversation.

#### 7. Distribution-Scope-Aware Packet Production

Render the report in **parallel scoped versions**:

- **`INTERNAL`** — Working underwriting note. Full reasoning, every indicator, every counter-indicator, the appetite-map verdict, the bias-check, the authority-check, and the **full Challenge Log** from Step 5.5. Signer block: `agency.signer_block.underwriter`
- **`BROKER-DISTRIBUTABLE`** — The cover note + outstanding-information request the broker can read. The verdict, the subjectivities, the outstanding-information list, the bind-timeline. PII-redacted (no SSN, no DOB, no medical, no internal commentary). Signer block: the producer or the underwriter per `agency.review_response_house_rules`. Drafted via the **Email Drafter** skill
- **`CARRIER-SUBMISSION`** — The broker-to-carrier outbound submission packet (when the agency is wholesaling to a carrier or a market). Insured snapshot, exposure summary, loss summary, requested terms, the broker's own underwriter narrative
- **`PEER-REVIEW`** — The peer / senior-underwriter review packet — the working note plus an explicit "what I'm asking for" from the senior underwriter (cover authority, decline authority, schedule-credit, exception). The **Challenge Log is the review agenda**, with every `OPEN` and `ESCALATED` challenge at the top. Signer block: `agency.signer_block.underwriter` plus the requested reviewer
- **`REGULATOR-RESPONSE`** — The redacted file copy a state DOI examiner can be handed. Removes the broker's commentary, removes producer-commission strategy, retains the appetite verdict, the bias-check, the **Challenge Log** (the evidence of meaningful human review before an AI-assisted decision), and the disposition. Signer block: `agency.signer_block.compliance-officer`. Routed through compliance counsel before submission

The skill produces every scope the user requested; if no scope is specified, the default bundle is `INTERNAL`.

#### 8. AMS / CRM Activity-Log Handoff

Append a short **AMS Activity-Log Block** after the working note:

```
ACTIVITY-LOG: [Insured] · [LoB] · [Eff date] · [Verdict] · [Premium est] · [Subjectivities count] · [Outstanding-info count] · [Underwriter]
```

Format adapts to `config.yml.agency.ams`:

- Applied Epic: `Activity Type | Description | Follow-up Date | Owner`
- AMS360: `Suspense | Description | Due | Assigned`
- HawkSoft: `Tag | Memo | Suspense | Owner`
- Vertafore: `Note Type | Detail | Diary | Producer`
- Salesforce: `Activity Subject | Comments | Due Date | Owner`

#### 9. Quality Check Before Returning

- `EFFICIENCY-RULE: <id>` stamped on the working note (or `NO EFFICIENCY RULE — UW DECISION` if unmapped); every inferred field carries `INFERRED — <source> — confidence <n>` provenance and the Outstanding Information list is rule-pruned with each ask citing its rule ID
- Every dimension scored with evidence and counter-evidence cited verbatim
- Composite score, dimension weights, and appetite-map verdicts shown explicitly
- Disposition matched to the carrier's `underwriting.disposition_thresholds`
- Authority-check shown — the recommended action stays within the underwriter's authority or is rendered as a routing recommendation
- Bias-check status shown
- **Challenge Log rendered** — all 12 failure modes in the taxonomy interrogated, each challenge graded `CRITICAL` / `MATERIAL` / `NOTED` and closed as `RESOLVED` (with verbatim citation) / `OPEN` / `ESCALATED`; `CRITIQUE-RULE: <id>` stamped (or `NO CRITIQUE RULE — CARRIER DEFAULT APPLIED`)
- **Critique gate cleared** — no `OPEN` `CRITICAL` challenge and `OPEN` `MATERIAL` challenges within `unresolved_challenge_ceiling`; otherwise the profile is `INTERNAL`-only and stamped `CRITIQUE GATE — HELD` with the blocking challenges and the named human it awaits
- **Decision-negative statement present** — the profile states on its face that it is a recommendation for a licensed human underwriter, not a decision
- Subjectivities, outstanding-information list, and surplus-lines / reinsurance triggers rendered
- Producer talk-track tuned to the verdict
- Distribution-scoped packets produced for every requested scope (default `INTERNAL`)
- Signer blocks pulled from `config.yml.agency.signer_block` per the resolved sender role
- AMS activity-log block in the user's AMS format
- PII-redaction applied on every non-`INTERNAL` scope per the rule above

### Output Requirements

- Structured report with sections: Insured Snapshot, Risk Assessment (per-dimension scoring), Claims Summary, Risk-Quality Scorecard, Composite Score and Appetite-Map Verdict, Bias-Check Status, Authority Check, **Challenge Log (Adversarial Self-Critique)**, Disposition and Recommended Action, Subjectivities, Outstanding Information, Surplus-Lines / Reinsurance Triggers, Producer Talk-Track, Distribution-Scoped Packets
- Challenge Log rendered as a numbered table (`# · Failure mode · Severity · Challenge · Disposition · Evidence cited / Reviewer routed to`) on the `INTERNAL`, `PEER-REVIEW`, and `REGULATOR-RESPONSE` scopes; suppressed on `BROKER-DISTRIBUTABLE` and `CARRIER-SUBMISSION`
- Risk-Quality Scorecard rates each dimension as `Favorable` / `Acceptable` / `Concerning` *plus* a 0–10 numeric score and an appetite-map verdict
- Professional formatting appropriate for the underwriting workbench and the peer-review queue
- Correct industry terminology (no generic business-speak); class codes / endorsement form numbers cited verbatim
- Disposition rendered explicitly with the carrier's verdict-to-action mapping
- Producer talk-track ≤ 200 words, producer-voice, drop-in-ready for the Producer Live-Call Copilot
- AMS activity-log block in the user's AMS format
- Distribution-scoped packets produced for every requested scope (default `INTERNAL`)
- Saved to `outputs/` if the user confirms

## Cross-References

- `operations/submission-intake-summarizer.md` — the upstream artifact this skill consumes
- `operations/loss-run-analyzer.md` — the upstream artifact this skill consumes for the Claims dimension
- `operations/renewal-review-brief.md` — the renewal-cycle counterpart that re-uses the dimension scoring and appetite-map verdict
- `sales/producer-live-call-copilot.md` — consumes the producer talk-track during the broker conversation
- `sales/policy-comparison-builder.md` — consumes the disposition and the subjectivities when the broker presents a comparison
- `admin/compliance-checklist-generator.md` — the per-state filing overlay
- `admin/ai-governance-model-card-generator.md` — the upstream AI rating / classification model card
- `_shared/email-drafter.md` — used to draft the broker-distributable cover note and the decline-to-quote letter
- `customer-service/coverage-explanation-letter.md` — the next-up artifact when the disposition turns into a written coverage position
- `operations/coi-compliance-reviewer.md` — referenced when the subjectivities include a certificate-of-insurance remediation

## Anti-Patterns

- Asserting an appetite verdict the carrier's appetite map does not support — broker-relationship damage and filed-rate risk
- Rendering a recommended action that exceeds the underwriter's authority — authority violation
- Using zip code, surname pattern, or country of origin as a *sole* indicator — proxy-discrimination exposure
- Treating an upstream AI rating score as a finding rather than a signal — model-risk and IN HB 1271 / NY DFS Reg 187 exposure
- Omitting the surplus-lines diligent-search affidavit when the verdict is admitted-decline → E&S placement — surplus-lines compliance violation
- Writing a quote with subjectivities the broker cannot satisfy by the bind date — broker-of-record relationship damage
- Skipping the filed-form / filed-rate check on a state with strict-form posture (NY / FL / TX / CA / IL / MA / WA / NJ) — filing violation
- **Running the challenger pass as a formality** — restating the profile's own conclusions in the challenge column instead of attempting to falsify them. A Challenge Log in which every mode returns "no issue found" on a non-trivial submission is evidence the gate was not actually run
- **Closing a challenge by weakening the disposition** rather than by citing evidence or escalating to a person — this launders an unresolved doubt into a softer verdict and destroys the audit trail
- **Suppressing, downgrading, or deleting a `CRITICAL` challenge to clear the gate** — the gate is cleared with evidence or by a named human, never by editing the record
- **Presenting the profile as a decision** rather than a recommendation to a licensed underwriter — the skill is decision-negative; asserting otherwise creates authority, adverse-action, and HITL-compliance exposure
- **Letting the Challenge Log cross into a `BROKER-DISTRIBUTABLE` or `CARRIER-SUBMISSION` scope** — internal deliberation handed to a broker or a market becomes negotiating leverage against the carrier and, on a decline, discoverable adverse-action material

## Example Output

> [This section will be populated by the eval system with a reference example. For now, run the skill with sample input to see output quality.]

## Versioning

- v4.0 (this version) — **Adversarial self-critique pass.** Added a **challenger gate (Step 5.5)** between the bias-check and the talk-track: a second, structurally adversarial pass whose purpose is to *falsify* the draft profile before it reaches the underwriter. Wired `config.yml.underwriting.critique_rules` (primary accuracy hook — `critique_required_scopes` with `DECLINE` / `REFER-UP` / `APPROVE-WITH-CREDIT` non-waivable, `failure_mode_register`, `challenge_severity_thresholds`, `unresolved_challenge_ceiling`, `critique_sampling_rate`), with a `NO CRITIQUE RULE — CARRIER DEFAULT APPLIED` fallback that **never skips the gate**. Added an explicit **decision-negative posture** statement (the skill renders a recommendation for a licensed human underwriter, never a bind / decline / rate / adverse-action decision) on the face of every profile. Added a **12-mode underwriting failure-mode taxonomy** (fabricated exposure fact, over-trusted inference, stale evidence, appetite-map overreach, expiring-program anchoring, broker-narrative anchoring, silent proxy discrimination via indicator *combinations*, authority drift, filed-rate / filed-form deviation, adverse-action defensibility, dimension-weight artifact, counter-evidence suppression), each with the challenger's named question. Added three-way challenge disposition (`RESOLVED` with verbatim citation / `OPEN` / `ESCALATED`), three-tier severity (`CRITICAL` / `MATERIAL` / `NOTED`), a **hard gate** (no advance to talk-track or packets while a `CRITICAL` challenge is `OPEN` or `MATERIAL` opens exceed the ceiling — the profile is held `INTERNAL`-only, stamped `CRITIQUE GATE — HELD`, naming the human it awaits), and an escalation path that forces `REFER-UP` regardless of composite score. Added the **Challenge Log as a first-class deliverable** — the peer-review agenda on `PEER-REVIEW`, the meaningful-human-review evidence on `REGULATOR-RESPONSE` (NAIC AI Model Bulletin / state HITL-on-adverse-action), suppressed on `BROKER-DISTRIBUTABLE` and `CARRIER-SUBMISSION`. Added a load-bearing-inference rule promoting any Step 0 `INFERRED` field that would flip the disposition to `INFERENCE — UW CONFIRM` regardless of confidence score, and an anchoring re-derivation (recompute the composite with expiring-program facts withheld and report whether the disposition survives). Added five anti-patterns (formality-run challenger, closing a challenge by weakening the disposition, suppressing a `CRITICAL` to clear the gate, presenting the profile as a decision, leaking the Challenge Log to a broker / market scope). Empirical basis cited: Roy & Singh, arXiv 2602.13213 (Jan-2026) — decision-negative HITL critic-agent architecture over 500 expert-validated commercial-underwriting cases; hallucination 11.3% → 3.8%, accuracy 92% → 96%; treated as directional evidence, with the carrier measuring its own rates via `critique_sampling_rate` post-hoc audit. **Strict superset of v3.0** — every v3.0 capability is preserved (the `uw_efficiency_rules` hook, Step 0 efficiency-rule lookup, the `EFFICIENCY-RULE` stamp, the inference-source chain with provenance and confidence, the rule-pruned Outstanding Information list, the UW-Confirm List, the `NO EFFICIENCY RULE — UW DECISION` fallback, the appetite map, per-dimension 0–10 scoring and weights, the five-verdict disposition matrix, the AI-bias / disparate-impact check, the authority-check, the producer talk-track, distribution-scoped parallel rendering, the AMS activity-log handoff, per-role signer blocks, and the v3.0 anti-patterns). **Scope guard:** the critique gate governs *how the recommendation is challenged and routed* — it never invents an exposure fact, a loss figure, an appetite verdict, a class code, an endorsement form number, or a coverage position to satisfy a challenge; an unresolvable challenge stays `OPEN` or `ESCALATED` to a person, and every `RESOLVED` closure carries a verbatim citation to primary evidence. **Dimension move:** Accuracy / defensibility uplift; efficiency held at 9 (the gate adds a bounded pass, and the load-bearing-inference promotion tightens rather than reopens the v3.0 inference chain).
- v3.0 — Efficiency pass. Wired `config.yml.underwriting.uw_efficiency_rules` (primary efficiency hook) — per-LoB × per-class-code × per-state × per-premium-band auto-clear-eligibility threshold library and inference-vs-ask matrix. Every risk-profile field is classified `required` / `inferable` / `confirm-after-infer` / `ask` against the carrier's rule library, with a rule-prescribed inference-source chain (upstream Submission Intake Summarizer v4.0 summary → upstream Loss Run Analyzer v3.0 analysis → prior-policy record → broker packet → AMS/BMS → D&B/Experian/Equifax credit feed → public-filing record → ISO PolicyFinder / NICB → SOC 2 → inspection-report feed → CAT-modeling feed → third-party aggregator) carrying explicit provenance and confidence. New Step 0 (efficiency-rule lookup) before fact extraction; `EFFICIENCY-RULE: <id>` stamp on the working note; `INFERRED — <source> — confidence <n>` provenance on every inferred field; rule-pruned Outstanding Information list (asks the broker only for `required` + `ask` + missing fields, each citing its rule ID); internal-only UW-Confirm List for sub-threshold inferences; `NO EFFICIENCY RULE — UW DECISION` fallback to v2.0 behavior for unmapped tuples. **Strict superset of v2.0** — every v2.0 capability is preserved (appetite map, per-dimension 0–10 scoring + weights, five-verdict disposition matrix, AI-bias / disparate-impact check, authority-check, producer talk-track, distribution-scoped parallel rendering, AMS activity-log handoff, per-role signer blocks, anti-patterns, expanded dimension library). Cross-references updated to consume Submission Intake Summarizer v4.0 and Loss Run Analyzer v3.0. **Dimension move:** Efficiency 7 → 9 (Step 0 lookup + inference-source chain + rule-pruned Outstanding Information + UW-Confirm List); all other dimensions held.
- v2.0 — Carrier or agency appetite map driven by `config.yml.underwriting.appetite_map` per LoB × class code × state with explicit verdicts (`IN-APPETITE` / `IN-APPETITE-WITH-CONDITIONS` / `OUT-OF-APPETITE` / `MUST-REFER-UP`); per-dimension 0–10 scoring with weights from `config.yml.underwriting.dimension_weights` (default 25/20/25/15/15) and the composite shown explicitly; five-verdict disposition matrix (`DECLINE` / `REFER-UP` / `APPROVE-WITH-CONDITIONS` / `APPROVE-AS-IS` / `APPROVE-WITH-CREDIT`) with verdict-to-action mapping including subjectivities, outstanding-information list, surplus-lines posture, and reinsurance triggers; AI-bias and disparate-impact check (CO SB 21-169 / NY DFS Reg 187 / IN HB 1271 / NAIC AI Model Bulletin / EU AI Act Annex III) with explicit `PASS` / `REVIEW NEEDED` / `BIAS RISK FLAGGED` status; authority-check against `config.yml.underwriting.authority_limits` so recommendations stay within the underwriter's authority or render as routing recommendations; producer talk-track tuned to the verdict (≤ 200 words, drop-in for Producer Live-Call Copilot); distribution-scoped parallel rendering (`INTERNAL` / `BROKER-DISTRIBUTABLE` / `CARRIER-SUBMISSION` / `PEER-REVIEW` / `REGULATOR-RESPONSE`) with per-scope redaction rules; AMS activity-log handoff in the user's AMS format (Applied Epic / AMS360 / HawkSoft / Vertafore / Salesforce); per-role signer blocks (underwriter / producer / executive / compliance-officer); cross-references to Submission Intake Summarizer, Loss Run Analyzer, Renewal Review Brief, Producer Live-Call Copilot, Policy Comparison Builder, Compliance Checklist Generator, AI Governance Model Card Generator, Email Drafter, Coverage Explanation Letter, COI Compliance Reviewer; explicit anti-patterns section; expanded dimension library (cyber NIST CSF maturity, ESG, AI-and-automation exposure, climate-transition, supply-chain, regulatory-action history)
- v1.0 — Insured Snapshot, Risk Assessment (5 dimensions: Operations / Financial / Claims / CAT / Emerging), Claims Summary, Risk Quality Scorecard (Favorable / Acceptable / Concerning), Underwriting Recommendations, Outstanding Questions. (Now superseded — every v1.0 capability is preserved in v2.0.)
