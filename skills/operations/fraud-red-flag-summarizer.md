...
time_saved: "~30 min/claim + faster SIU referral cycle + rule-pre-resolved review methodology"
version: 3.0
...
- Load `config.yml` for:
  ...
  - **`claims.fraud_efficiency_rules`** *(new in v3.0 — primary efficiency hook)* — per-LoB × per-state × per-severity-band auto-resolve-eligibility threshold library and infer-vs-ask matrix for every *review-methodology parameter* the skill would otherwise stop and request, so the review baseline is pre-resolved rather than re-elicited. Each parameter the indicator analysis depends on (which per-LoB indicator library applies, the SIU-referral threshold for the LoB, the mandatory-reporting-state determination, the severity × confidence weighting set, the NICB-membership / ISO-ClaimSearch posture, the bias-test methodology, the distribution-scope default) is classified `required` / `inferable` / `confirm-after-infer` / `ask`. **Inference-source chain (rule-prescribed in order):** the line of business and loss state named on the file → `claims.lines_of_business` + the per-LoB indicator subset → `knowledge-base/fraud-indicators/` carrier library (NICB / CAIF fallback where a slot is missing) → `claims.siu_referral_thresholds[<LoB>]` → `claims.mandatory_reporting_states` → `claims.siu_program` NICB / ISO-ClaimSearch / fraud-model posture → `claims.authority_limits` for the handler level. Each inference carries explicit provenance and confidence; parameters at or above the carrier's accepted confidence threshold are marked `INFERRED — <source> — confidence <n>`; parameters below threshold are marked `INFERENCE — ADJUSTER CONFIRM` and held on an internal Adjuster-Confirm List rather than blocking the report. The **Missing Facts list** is **rule-pruned** — it surfaces only inputs the rule classifies as `required` AND `ask` (cannot be inferred from any chain source AND genuinely needed to advance the disposition) AND missing; every entry cites the rule ID. `NO EFFICIENCY RULE — ADJUSTER DECISION` flag for unmapped LoB / state / severity-band tuples (default to ask, v2.0 behavior). **Scope guard:** the hook governs which *review methodology* applies — it never infers a *claim fact* (an indicator's evidentiary basis, the verbatim evidence cite, a claimant or party identity, a claimed / paid / reserved amount, a NICB ISO ClaimSearch hit, a fraud finding, or a severity / confidence verdict on a specific indicator); the claim file remains untrusted data, every indicator is still evidenced verbatim from the file, and every v2.0 fail-graceful refusal and Daubert / confidence discipline in the Anti-Patterns section still applies verbatim. Modeled on the FNOL v3.0, Loss Run Analyzer v3.0, and COI Compliance Reviewer v3.0 efficiency-rule patterns.
...
Resolve the applicable `config.yml.claims.fraud_efficiency_rules` entry per (LoB × loss-state × severity-band) **before** parsing the claim file. Stamp the rule ID at the top of every distribution-scoped packet (`EFFICIENCY-RULE: <id>`). The resolved row decides which review-methodology parameters (per-LoB indicator library, SIU-referral threshold, mandatory-reporting-state determination, severity × confidence weighting, NICB / ISO-ClaimSearch posture, bias-test methodology, distribution-scope default) are `required`, which are `inferable` (and from which chain source), which need `confirm-after-infer` adjuster sign-off, and which must be `ask`ed. Walk the inference-source chain — anchored on the LoB and loss state on the file — for every `inferable` / `confirm-after-infer` parameter and record source and confidence so the review baseline is pre-resolved rather than re-elicited. If no rule matches, set `NO EFFICIENCY RULE — ADJUSTER DECISION` and proceed with v2.0 baseline behavior (do not block the report). This step governs which *review methodology* applies only — it never infers a *claim fact* (an indicator's evidentiary basis, the verbatim evidence cite, a party identity, a claimed / paid / reserved amount, a NICB ISO ClaimSearch hit, a fraud finding, or a per-indicator severity / confidence verdict); the claim file remains untrusted data, every indicator is still evidenced verbatim, and the Daubert / confidence discipline and the Anti-Patterns section still apply verbatim.
...
- Structured report with sections: Efficiency-Rule Stamp (`EFFICIENCY-RULE: <id>`), Summary, Timeline, Indicators Triggered, Composite Score and Threshold, Bias-Check Status, Authority Check, Recommended Action, Investigative Next Steps, Distribution-Scoped Packets (inferred review-methodology parameters appear marked `INFERRED — <source> — confidence <n>`, and sub-threshold inferences go to an internal Adjuster-Confirm List)
- The Missing Facts list is rule-pruned — it surfaces only inputs the efficiency rule classifies as `required` AND `ask` AND missing, each citing its rule ID; methodology parameters resolvable from the inference-source chain are pre-resolved rather than re-elicited
...
- Using the `claims.fraud_efficiency_rules` hook to infer a *claim fact* — an indicator's evidentiary basis, a verbatim evidence cite, a party identity, a claimed / paid / reserved amount, a NICB ISO ClaimSearch hit, a fraud finding, or a per-indicator severity / confidence verdict. The hook resolves *review methodology* only; the claim file stays untrusted and every indicator stays evidenced verbatim from the file
...
- v3.0 (2026-06-29) — Efficiency pass. Wired `config.yml.claims.fraud_efficiency_rules` (primary efficiency hook) — per-LoB × per-state × per-severity-band auto-resolve-eligibility threshold library and infer-vs-ask matrix over the review's *methodology parameters* (per-LoB indicator library, SIU-referral threshold, mandatory-reporting-state determination, severity × confidence weighting, NICB / ISO-ClaimSearch posture, bias-test methodology, distribution-scope default) so the review baseline is pre-resolved rather than re-elicited. Rule-prescribed inference-source chain (LoB + loss state on the file → lines_of_business + per-LoB indicator subset → KB fraud-indicator library / NICB-CAIF fallback → siu_referral_thresholds[LoB] → mandatory_reporting_states → siu_program posture → authority_limits) with explicit provenance and confidence. New Step 0 (efficiency-rule lookup) before claim-file parsing; `EFFICIENCY-RULE: <id>` stamp on every distribution-scoped packet; `INFERRED — <source> — confidence <n>` provenance; rule-pruned Missing Facts list (surfaces only `required` + `ask` + missing inputs, each citing its rule ID); internal Adjuster-Confirm List for sub-threshold inferences; `NO EFFICIENCY RULE — ADJUSTER DECISION` fallback to v2.0 behavior for unmapped tuples. **Scope guard:** the hook governs which *review methodology* applies — it never infers a *claim fact* (an indicator's evidentiary basis, the verbatim evidence cite, a party identity, a claimed / paid / reserved amount, a NICB ISO ClaimSearch hit, a fraud finding, or a per-indicator severity / confidence verdict); the claim file remains untrusted data verified verbatim, and every v2.0 fail-graceful refusal and the Daubert / confidence discipline still apply verbatim. **Strict superset of v2.0** — every v2.0 capability is preserved (9-LoB indicator library with NICB / CAIF codes, cross-LoB universal-indicator set with AI-image-generation detection and telematics / IoT / scene-data inconsistency, severity × confidence composite scoring with explicit threshold comparison, AI-bias / disparate-impact check, authority-check, five-scope distribution-aware parallel rendering, mandatory-reporting-state list, concurrent-investigation guardrail, BSA / FinCEN SAR trigger, reservation-of-rights / EUO / IME / NICB / sub-rosa next-step recommendations, per-role signer blocks, anti-patterns). **Dimension move:** Efficiency 7 → 9 (Step 0 lookup + methodology inference chain + rule-pruned Missing Facts list + Adjuster-Confirm List); all other dimensions held.
- v2.0 — Per-LoB indicator library spanning 9 LoBs (personal auto, personal property, commercial auto / trucking, commercial property, workers' comp, GL, professional / medical-pro, A&H / Medicare, life) with NICB / CAIF reference codes; cross-LoB universal-indicator set including AI-image-generation detection and telematics / IoT / scene-data inconsistency; severity × confidence composite scoring with explicit threshold comparison from `config.yml.claims.siu_referral_thresholds`; AI-bias and disparate-impact check (CO SB 21-169 / NY DFS Reg 187 / IN HB 1271 / NAIC AI Model Bulletin) with explicit `PASS` / `REVIEW NEEDED` / `BIAS RISK FLAGGED` status; authority-check against `config.yml.claims.authority_limits` so recommendations stay within the handler's authority; distribution-scoped parallel rendering (`INTERNAL` / `SIU-DISTRIBUTABLE` / `COUNSEL-DISTRIBUTABLE` / `NICB-REFERRAL` / `STATE-FRAUD-BUREAU`) with per-scope redaction rules; mandatory-reporting-state list defaulted to NAIC Fraud Prevention Model Act adopters and configurable per `config.yml.claims.mandatory_reporting_states`; concurrent-investigation guardrail (never delay the claim solely on the SIU referral); BSA / FinCEN SAR trigger for life claims; reservation-of-rights / EUO / IME / NICB / sub-rosa next-step recommendations gated on authority; per-role signer blocks (claims-handler / siu-lead / compliance-officer); cross-references to Claims Reserve Recommender, Claims Narrative Drafter, Coverage Explanation Letter, AI Governance Model Card Generator, Compliance Checklist Generator, CAT Claim Surge Triage, Email Drafter; explicit anti-patterns section
